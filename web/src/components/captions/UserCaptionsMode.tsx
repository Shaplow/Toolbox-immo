      ...
      <div className="max-w-xl mx-auto px-4 py-10 space-y-6">
        ...
        <StepCard number={1} title="Style de sous-titres">
          {presetsLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-2">Aucun preset disponible — contactez votre administrateur.</p>
          ) : (
              ...
              {presets.map((p) => (
                ...
                >
                  ...
                  <div>
                    ...
                    {p.isBuiltin && (
                      <p className="text-[10px] text-danger-200 mt-0.5">Style intégré</p>
                    )}
                  </div>
                  {selectedPresetId === p.id && <span className="ml-auto text-danger-600 text-sm">✓</span>}
                </button>
              ))}
            </div>
          )}
        ...
        <StepCard number={2} title="Votre vidéo">
          <label className={`flex flex-col items-center gap-2 p-6 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
            videoFile ? "border-danger-200 bg-danger-50" : "border-border hover:border-border bg-white"
          }`}>
            <Upload size={20} className={videoFile ? "text-danger-600" : "text-muted-foreground"} />
            <span className="text-sm font-medium text-foreground">
              {videoFile ? videoFile.name : "Cliquer pour choisir une vidéo"}
            </span>
            {videoFile && (
              <span className="text-xs text-danger-700">{(videoFile.size / 1_000_000).toFixed(1)} Mo</span>
            )}
        ...
        <StepCard number={3} title="Sous-titres (.srt)">
          <label className={`flex flex-col items-center gap-2 p-6 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
            (subsFile || captions.length > 0) ? "border-danger-200 bg-danger-50" : "border-border hover:border-border bg-white"
          }`}>
            <FileText size={20} className={(subsFile || captions.length > 0) ? "text-danger-600" : "text-muted-foreground"} />
            <span className="text-sm font-medium text-foreground">
              {subsFile
                ? subsFile.name
                : captions.length > 0
        ...
        <StepCard number={4} title="Qualité d&apos;export">
          <select
            ...
            className="w-full border border-border rounded-xl px-3 py-2.5 text-sm text-foreground bg-white focus:outline-none focus:ring-2 focus:ring-danger-200"
          >
        ...
        {renderProgress >= 0 && (
          <div className="w-full bg-muted rounded-full h-2 overflow-hidden">
            <div className="bg-danger-600 h-2 rounded-full transition-all duration-500"
              style={{ width: `${Math.round(renderProgress * 100)}%` }} />
          </div>
        )}

        {/* Status */}
        <div className={`text-sm text-center ${message.startsWith("Erreur") ? "text-red-500" : "text-muted-foreground"}`}>
          {message}
        </div>
