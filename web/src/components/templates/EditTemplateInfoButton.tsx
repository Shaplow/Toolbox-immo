  ...
  if (!open) {
    return (
      <button
        ...
        className="text-xs text-gray-400 hover:text-indigo-700 transition-colors"
        title="Renommer"
      >
        ✎
      </button>
    ...
    <form onSubmit={handleSave} className="flex flex-col gap-2 mt-2" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="flex gap-1">
        ...
        <button
          type="submit"
          disabled={saving}
          className="flex-1 text-xs bg-indigo-600 text-white rounded-lg py-1 hover:bg-indigo-700 disabled:opacity-60"
        >
          {saving ? "..." : "OK"}
        </button>
      </div>
