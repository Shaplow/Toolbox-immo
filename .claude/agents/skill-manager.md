---
name: skill-manager
description: Audit and maintain Toolbox Immo skills, agents, and repository guidance. Use when updating existing SKILL.md files, creating new skills or agents, checking drift after refactors, refreshing instructions, or verifying that CLAUDE context still matches the codebase. Trigger keywords — "audit skills", "skill drift", "update agents", "refresh instructions", "check CLAUDE.md".
model: sonnet
---
...
- `.github/agents/*.agent.md` (legacy Copilot backups — only fix factual drift, do not delete)
...
3. List `.claude/agents/` and `.claude/commands/`.
4. List `.claude/skills/` and `.github/instructions/`.
5. If a referenced document does not exist, note that explicitly instead of assuming an equivalent file exists.
...
- Description generation features: `web/src/components/description/`, related routes and helpers
...
Check whether `.claude/CLAUDE.md` still matches the repository:
...
- git discipline section still matches the user's actual workflow
...
For each `.claude/skills/*/SKILL.md`, verify that:
...
For each `.claude/agents/*.md`, verify that:
...
- the model field uses a valid Claude Code model name (sonnet, opus, haiku) — not Copilot-style names
...
Look for complex or fragile areas that may need a dedicated skill or agent:
...
A good `.claude/agents/<name>.md` must include:

- valid frontmatter with `name`, `description`, and (optionally) `model` and `tools`
- trigger-rich description text with clear `Use when` language and trigger keywords
...
- use Copilot-style model names like `Claude Sonnet 4.6 (copilot)` — use `sonnet`, `opus`, or `haiku`
