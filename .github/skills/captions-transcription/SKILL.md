...
**RunPod worker** (`runpod_worker.py`): handles `job_type: "caption"` jobs. Expects `video_url`, `srt_content`, `config`, `preview_mode`, `output_key`.
...
- **`web/src/lib/captionsEngine.ts`**: `normalizeCaptionConfig()`. Always forces `engine: "ass"`. The function is now a thin normalizer — `inferCaptionsEngine` no longer exists and there is no Cairo or `line_height_mode` logic remaining.
...
| Worker | `render-engine/engine/ass_writer.py` | ASS subtitle generation |
